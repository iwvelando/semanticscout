...
class ReportGenerator:
    ...
    def _format_job_entry(self, job: Job, index: int) -> list[str]:
        ...
        if job.matched_city:
            if job.matched_city == "Remote":
                lines.append(f"| **Matched** | 🏠 Remote |")
            elif job.matched_city == "Anywhere":
                lines.append(f"| **Matched** | 🌍 Anywhere (geo filter disabled) |")
            else:
                distance_str = f" ({job.distance_miles:.1f} mi)" if job.distance_miles else ""
                lines.append(f"| **Matched** | 📍 {job.matched_city}{distance_str} |")
        elif job.is_remote:
    ...
    def print_summary(self, jobs: list[Job]) -> None:
        ...
        for i, job in enumerate(sorted_jobs[:10], 1):
            ...
            if job.score_reasoning:
                print(f"   💡 {job.score_reasoning[:80]}...")
            print()
    ...
    def _build_slack_summary_blocks(
        ...
        if not jobs:
            blocks.append({
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": "📭 No jobs scoring above the threshold were found today."
                }
            })
            return blocks
        ...
        blocks.append({
            ...
            "elements": [
                {
                    "type": "mrkdwn",
                    "text": "Generated by *Semantic Scout* 🔍"
                }
            ]
        })
    ...
    def _build_mention_text(self, jobs: list[Job], date: datetime) -> str:
        ...
        if jobs:
            parts.append(
                f"🎯 Semantic Scout found {len(jobs)} high-scoring opportunities "
                f"for {date.strftime('%B %d, %Y')}"
            )
        else:
            parts.append(
                f"📭 No high-scoring jobs found for {date.strftime('%B %d, %Y')}"
            )
